	...
	internal static class Program
	{
		...
		private static void Main()
		{
			try
			{
				...
				_logger.Info(message: "Anwendung gestartet.");

				// Starts the main form of the application
				using BlackAndWhiteForm mainForm = new();
			...
			catch (InvalidOperationException ex)
			{
				...
				_logger.Error(exception: ex, message: message);
				LogError(ex);
		...
		private static void LogError(Exception ex)
		{
			...
			Console.WriteLine(value: $@"Fehler: {ex.Message} {ex.StackTrace}");
		}
